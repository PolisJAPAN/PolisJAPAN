...
/**
 * 指定URLへJSON POSTし、JSONレスポンスを取得する。
 * @param {string} url - 送信先URL。
 * @param {Object} data - POSTするJSONデータ（オブジェクト）。
 * @param {RequestInit} [options={}] - fetchオプション（headers等を上書き可）。
 * @returns {Promise<Object>} - パース済みJSONオブジェクト。
 * @throws {Error} - HTTPエラーまたはJSONパース失敗時。
 */
async function fetchJsonPostAsJson(url, data, options = {}) {
    const headers = {
